...
cat $SUPPORT_DIR/makedevs/device_table.txt > $BUILD_DIR/_device_table.txt
echo "/var/lib/sshd	d	700	0	3	-	-	-	-	-" >> $BUILD_DIR/_device_table.txt
...
echo "tar -cvzf $IMAGES_DIR/StaticNux-$CONFIG_STATICNUX_VERSION.tar.gz bin boot dev etc lib lib64 linuxrc media mnt opt proc root run sbin sys tmp usr var" >> $BUILD_DIR/_fakeroot.fs
