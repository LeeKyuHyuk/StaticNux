...
CONFIG_PKG_VERSION="StaticNux x86_64 2021.09"
CONFIG_BUG_URL="https://github.com/LeeKyuHyuk/StaticNux/issues"
...
step "[1/25] Create root file system directory."
rm -rf $ROOTFS_DIR
...
step "[2/25] Creating Essential Files and Symlinks"
# Create /etc/passwd
...
nogroup:x:65534:
EOF
echo "Welcome to StaticNux on the Docker" > $ROOTFS_DIR/etc/issue
...
step "[8/25] General Network Configuration"
...
step "[9/25] The Bash Shell Startup Files"
...
step "[10/25] Creating the /etc/fstab File"
...
step "[11/25] Setting Local Timezone"
...
install -m 754 $SUPPORT_DIR/openssh/sshd $ROOTFS_DIR/etc/init.d/S30sshd
rm -rf $BUILD_DIR/openssh-8.7p1


step "[15/25] M4 1.4.19"
...
make -j$PARALLEL_JOBS DESTDIR=$ROOTFS_DIR install -C $BUILD_DIR/m4-1.4.19
rm -rf $BUILD_DIR/m4-1.4.19

step "[16/25] Libtool 2.4.6"
...
make -j$PARALLEL_JOBS DESTDIR=$ROOTFS_DIR install -C $BUILD_DIR/libtool-2.4.6
rm -rf $BUILD_DIR/libtool-2.4.6

step "[17/25] Autoconf 2.71"
...
make -j$PARALLEL_JOBS DESTDIR=$ROOTFS_DIR install -C $BUILD_DIR/autoconf-2.71
rm -rf $BUILD_DIR/autoconf-2.71

step "[18/25] Automake 1.16.4"
...
make -j$PARALLEL_JOBS DESTDIR=$ROOTFS_DIR install -C $BUILD_DIR/automake-1.16.4
mkdir -p $ROOTFS_DIR/usr/share/aclocal
...
make -j$PARALLEL_JOBS DESTDIR=$ROOTFS_DIR install -C $BUILD_DIR/make-4.3
rm -rf $BUILD_DIR/make-4.3

step "[20/25] Bison 3.7.6"
...
make -j$PARALLEL_JOBS DESTDIR=$ROOTFS_DIR install -C $BUILD_DIR/bison-3.7.6
rm -rf $BUILD_DIR/bison-3.7.6

step "[21/25] Gawk 5.1.0"
...
make -j$PARALLEL_JOBS DESTDIR=$ROOTFS_DIR install -C $BUILD_DIR/gawk-5.1.0
rm -rf $BUILD_DIR/gawk-5.1.0

step "[22/25] Linux 5.13.13 API Headers"
...
make -j$PARALLEL_JOBS ARCH=$CONFIG_LINUX_ARCH INSTALL_HDR_PATH=$ROOTFS_DIR headers_install -C $BUILD_DIR/linux-5.13.13
rm -rf $BUILD_DIR/linux-5.13.13

step "[23/25] Binutils 2.37"
...
make -j$PARALLEL_JOBS DESTDIR=$SYSROOT_DIR install -C $BUILD_DIR/binutils-2.37/binutils-build
make -j$PARALLEL_JOBS DESTDIR=$ROOTFS_DIR install -C $BUILD_DIR/binutils-2.37/binutils-build
rm -rf $BUILD_DIR/binutils-2.37

step "[24/25] Gcc 11.2.0"
...
sed -i 's@RAW_CXX_FOR_TARGET="$CXX_FOR_TARGET"@RAW_CXX_FOR_TARGET="$CXX_FOR_TARGET -nostdinc++"@g' $BUILD_DIR/gcc-11.2.0/configure
sed -e '/static.*SIGSTKSZ/d' \
    -e 's/return kAltStackSize/return SIGSTKSZ * 4/' \
    -i $BUILD_DIR/gcc-11.2.0/libsanitizer/sanitizer_common/sanitizer_posix_libcdep.cpp
case $CONFIG_LINUX_ARCH in
  x86_64)
    sed -i.orig '/m64=/s/lib64/lib/' $BUILD_DIR/gcc-11.2.0/gcc/config/i386/t-linux64
  ;;
esac
...
make -j$PARALLEL_JOBS CC_FOR_TARGET="$TOOLS_DIR/bin/$CONFIG_TARGET-gcc" CXX_FOR_TARGET="$TOOLS_DIR/bin/$CONFIG_TARGET-g++" AS_FOR_TARGET="$TOOLS_DIR/bin/$CONFIG_TARGET-as" LD_FOR_TARGET="$TOOLS_DIR/bin/$CONFIG_TARGET-ld" -C $BUILD_DIR/gcc-11.2.0/gcc-build
make -j$PARALLEL_JOBS DESTDIR=$SYSROOT_DIR install -C $BUILD_DIR/gcc-11.2.0/gcc-build
make -j$PARALLEL_JOBS DESTDIR=$ROOTFS_DIR install -C $BUILD_DIR/gcc-11.2.0/gcc-build
...
rm -rf $ROOTFS_DIR/usr/lib/gcc/$CONFIG_TARGET/11.2.0/include-fixed/bits/
ln -v -sf ../usr/bin/cpp $ROOTFS_DIR/lib
ln -v -sf gcc $ROOTFS_DIR/usr/bin/cc
install -v -dm755 $ROOTFS_DIR/usr/lib/bfd-plugins
ln -sfv ../../libexec/gcc/$CONFIG_TARGET/11.2.0/liblto_plugin.so $ROOTFS_DIR/usr/lib/bfd-plugins/
rm -rf $BUILD_DIR/gcc-11.2.0

step "[25/25] Perl 5.34.0"
extract $SOURCES_DIR/perl-5.34.0.tar.gz $BUILD_DIR
tar xzf $SOURCES_DIR/perl-cross-1.3.6.tar.gz --strip-components=1 -C $BUILD_DIR/perl-5.34.0
export BUILD_ZLIB=False
export BUILD_BZIP2=1
( cd $BUILD_DIR/perl-5.34.0 && \
	HOSTCC='/usr/bin/gcc' \
	./configure \
	--target=$CONFIG_TARGET \
	--target-tools-prefix=$TOOLS_DIR/bin/$CONFIG_TARGET- \
	--prefix=/usr \
	-Dld="$TOOLS_DIR/bin/$CONFIG_TARGET-gcc" \
	-Dccflags="-D_LARGEFILE_SOURCE -D_LARGEFILE64_SOURCE -D_FILE_OFFSET_BITS=64 -Os" \
	-Dldflags=" -lm" \
	-Dmydomain="" \
	-Dmyhostname="noname" \
	-Dmyuname="$CONFIG_PKG_VERSION" \
	-Dosname=linux \
	-Dosvers=4.18.10 \
	-Dperladmin=root )
make -j$PARALLEL_JOBS -C $BUILD_DIR/perl-5.34.0
make -j$PARALLEL_JOBS DESTDIR=$ROOTFS_DIR install.perl install.sym -C $BUILD_DIR/perl-5.34.0
unset BUILD_ZLIB BUILD_BZIP2
rm -rf $BUILD_DIR/perl-5.34.0
